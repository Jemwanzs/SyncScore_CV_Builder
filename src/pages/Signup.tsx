...
import { signup } from '@/lib/auth';
import { useToast } from '@/hooks/use-toast';
import './Signup.css'; // Floating CV CSS
...
export default function Signup() {
  ...
  const validatePassword = (pwd: string) => {
    ...
    if (pwd.length < 6) { setPasswordError('Password must be at least 6 characters'); return false; }
    setPasswordError(''); return true;
  };

  const validateConfirmPassword = (pwd: string) => {
    if (!pwd) { setConfirmPasswordError('Please confirm your password'); return false; }
    if (pwd !== password) { setConfirmPasswordError('Passwords do not match'); return false; }
  ...
  if (showPayment) {
    return (
      ...
      >
        ...
        <Card className="relative z-20 w-full max-w-md p-8">
          <h1 className="text-3xl font-bold mb-2">MPESA Payment</h1>
          <p className="text-muted-foreground mb-6">Complete your payment to activate your account</p>
          <div className="space-y-6">
          <div className="bg-primary/10 border border-primary/20 p-5 rounded-lg">
              <p className="text-sm font-medium mb-3">Payment Instructions:</p>
              <ol className="space-y-2 text-sm">
                ...
                <li className="flex gap-2">
                  <span className="font-bold">2.</span>
                  <span>Call <strong>Admin</strong> for email activation</span>
                </li>
                <li className="flex gap-2">
                  <span className="font-bold">3.</span>
                  <span>Enter your MPESA payment code below</span>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                required
              />
              <p className="text-xs text-muted-foreground">Enter the MPESA confirmation code</p>
            </div>
  ...
  // --- Signup form ---
  return (
    <div className="min-h-screen relative flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary/10 via-background to-secondary/10 p-4">
      {cvImages.map((img, i) => (
        <img
          key={i}
          src={img}
      ...
      <Card className="relative z-20 w-full max-w-md p-8 bg-white/90 backdrop-blur-md shadow-xl rounded-2xl">
        <h1 className="text-3xl font-bold mb-2">Create Account</h1>
        <p className="text-muted-foreground mb-6">Sign up to build your CV</p>
        <form onSubmit={handleSignup} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="email">Email</Label>
            <Input
          ...
          <Label htmlFor="password">
            Password (<span className="text-gray-500 italic">min. 6 characters</span>)
          </Label>
            <Input
              id="password"
              type="password"
