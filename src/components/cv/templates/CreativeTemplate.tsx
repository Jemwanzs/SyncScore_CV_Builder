    ...
    <div className="bg-white text-gray-900 w-[210mm] min-h-[297mm] mx-auto flex" style={{ fontFamily: 'Inter, sans-serif', fontSize: '11pt' }}>
      ...
      <aside className="w-1/3 p-8" style={{ backgroundColor: colorScheme.primary, color: 'white' }}>
        <div className="mb-6">
          ...
          <p className="text-[8pt] font-light tracking-widest uppercase opacity-70 mb-2">CV</p>
...
export const CreativeTemplate = ({ data }: CreativeTemplateProps) => {
  ...
  const formatDate = (start?: string, end?: string, current?: boolean) => {
    if (!start) return '';
    const startFmt = new Date(start).toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
    const endFmt = current
      ? 'Present'
      : end
  ...
  return (
    <div
      className="bg-white text-gray-900 w-[210mm] min-h-[297mm] mx-auto flex"
    ...
    >
      ...
      >
        {/* Name + Title */}
        <div className="mb-6">
          <h1 className="font-bold mb-1 break-words" style={{ fontSize: '22pt' }}>
            {data.personalInfo.fullName}
          </h1>
          <p className="text-[8pt] tracking-widest uppercase opacity-70 mb-2">CV</p>
          <p className="opacity-90" style={{ fontSize: '12pt' }}>
        ...
        <div className="mb-6">
          <h2
            className="font-bold mb-3 pb-2 border-b border-white/30"
            style={{ fontSize: '14pt' }}
          >
            Contact
          </h2>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-1.5" style={{ fontSize: '9pt' }}>
              {data.hobbies.map((hob) => (
                <div key={hob.id}>
                  <p className="font-medium">{hob.name}</p>
                  {hob.description && (
                    <p className="opacity-80 text-[8pt]">{hob.description}</p>
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4">
              {data.experience.map((exp) => (
                <div key={exp.id}>
                  <div className="flex justify-between items-start mb-1.5">
                    <div>
                      <h3 className="font-semibold text-[12pt]">{exp.position}</h3>
                      <p className="font-medium text-[11pt]" style={{ color: colorScheme.secondary }}>
                        {exp.company}
                      </p>
                    </div>

                    <span className="text-gray-600 whitespace-nowrap ml-4 text-[9pt]">
                      {formatDate(exp.startDate, exp.endDate, exp.current)}
                    </span>
                  </div>

                  <div className="text-gray-700 text-[10pt]">
                    {renderRichText(exp.description)}
                  </div>
                </div>
              ))}
        ...
        {data.education.length > 0 && (
          <section className="mb-5 cv-section">
            ...
            <div className="space-y-3">
              {data.education.map((edu) => (
                <div key={edu.id}>
                  <h3 className="font-semibold text-[12pt]">
                  ...
                  <p className="text-gray-600 text-[9pt]">
                    {formatDate(edu.startDate, edu.endDate, edu.current)}
                    {edu.gpa && ` • GPA: ${edu.gpa}`}
                  </p>
                </div>
        ...
        {data.publications.length > 0 && (
          <section className="mb-5 cv-section">
            ...
            <div className="space-y-3">
              {data.publications.map((pub) => (
                <div key={pub.id}>
                  <h3 className="font-semibold text-[11pt]">{pub.title}</h3>

                  <p className="text-[10pt]" style={{ color: colorScheme.secondary }}>
                    {pub.publisher} •{' '}
                  ...
                  <div className="text-gray-700 mt-1 text-[10pt]">
                    {renderRichText(pub.description)}
                  </div>
                </div>
              ))}
        ...
        {data.awards.length > 0 && (
          <section className="mb-5 cv-section">
            ...
            <div className="space-y-3">
              {data.awards.map((award) => (
                <div key={award.id}>
                  <h3 className="font-semibold text-[11pt]">{award.title}</h3>
        ...
        {data.volunteering.length > 0 && (
          <section className="mb-5 cv-section">
            ...
            <div className="space-y-3">
              {data.volunteering.map((vol) => (
                <div key={vol.id}>
                  <h3 className="font-semibold text-[11pt]">{vol.role}</h3>
          ...
          <section className="mb-5 cv-section">
            ...
            <div className="space-y-2.5">
              {data.references.map((ref) => (
                ...
                >
                  <h3 className="font-semibold text-[10pt]">{ref.name}</h3>

                  <p className="text-[9pt]" style={{ color: colorScheme.secondary }}>
                    {ref.title} {ref.company}
                  </p>

                  <p className="text-gray-600 text-[8pt]">{ref.relationship}</p>

                  <div className="mt-1.5 space-y-0.5 text-gray-700 text-[8pt]">
                    {ref.email && <p>{ref.email}</p>}
                    {ref.phone && <p>{ref.phone}</p>}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  {section.items.map((item) => (
                    <div key={item.id}>
                      <div className="flex justify-between items-start mb-1.5">
                        <div>
                          <h3 className="font-semibold text-[12pt]">{item.title}</h3>

                          {item.subtitle && (
                            <p
                              className="font-medium text-[11pt]"
                        ...
                        {(item.startDate || item.endDate) && (
                          <span className="text-[9pt] text-gray-600 whitespace-nowrap ml-4">
